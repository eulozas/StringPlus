...
// вычисления перенести в отдельную переменную добавить ширину и занести в
// переменные типа пересмотреть функции всех типов
void handler_fegEG(const char** ptr_str, FormatSpecifier* token,
                   va_list* args) {
  ParseFloat float_value;
