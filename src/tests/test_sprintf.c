...
START_TEST(test_int_plus_flag) {
  ...
  sprintf(buf1, "test:%% %+d %+i", 42, 42);
  s21_sprintf(buf2, "test:%% %+d %+i", 42, 42);
...
START_TEST(test_float_precision) {
  char buf1[BUF_SIZE], buf2[BUF_SIZE];
  sprintf(buf1, "%.2f %9.4f %#f %+f % f %+12.4f %012.6f %Lf %#.0f", 3.14159, 3.0000435, 3.0, 3.14159, 3.14159, 0.004356, -3.14159, (long double)3.1234566123441234546876876876, 12.12);
  s21_sprintf(buf2, "%.2f %9.4f %#f %+f % f %+12.4f %012.6f %Lf %#.0f", 3.14159, 3.0000435, 3.0, 3.14159, 3.14159, 0.004356, -3.14159, (long double)3.1234566123441234546876876876, 12.12);
  ck_assert_str_eq(buf1, buf2);
}
END_TEST
...
START_TEST(test_exponential_format) {
  char buf1[BUF_SIZE], buf2[BUF_SIZE];
  sprintf(buf1, "%.3e %.3E %#e %#E %-#10.3E %020.10e %e %e %e %e", 12345.6789, 12345.6789, 0.01, 1.0, -0.0000123, -0.0000123, 0.000000000002, 0.000, 999999.99, 0.0009999);
  s21_sprintf(buf2, "%.3e %.3E %#e %#E %-#10.3E %020.10e %e %e %e %e", 12345.6789, 12345.6789, 0.01, 1.0, -0.0000123, -0.0000123, 0.000000000002, 0.000, 999999.99, 0.0009999);
  ck_assert_str_eq(buf1, buf2);
}
END_TEST
...
START_TEST(test_g_format) {
  char buf1[BUF_SIZE], buf2[BUF_SIZE];
  sprintf(buf1, "%g %.4g %#g %G %g %#10.5g %.2g %#.2g %.1g %.0g %.4g %.10g %g", 123.456, 0.000123456, 123.0, 1234567.89, 1234567.89, 3.14, 0.0001, -0.0001, 0.0001, 0.0001, 12.0, 1.02, 123456789123.0);
  s21_sprintf(buf2, "%g %.4g %#g %G %g %#10.5g %.2g %#.2g %.1g %.0g %.4g %.10g %g", 123.456, 0.000123456, 123.0, 1234567.89, 1234567.89, 3.14, 0.0001, -0.0001, 0.0001, 0.0001, 12.0, 1.02, 123456789123.0);
  ck_assert_str_eq(buf1, buf2);
}
END_TEST
...
START_TEST(test_string_and_char) {
  char buf1[BUF_SIZE], buf2[BUF_SIZE];
  sprintf(buf1, "%s %10s %.3s %-10.4s %c %5c %-5c", "hello", "hi", "abcdef", "testing", 'A', 'f', 'B');
  s21_sprintf(buf2, "%s %10s %.3s %-10.4s %c %5c %-5c", "hello", "hi", "abcdef", "testing", 'A', 'f', 'B');
  ck_assert_str_eq(buf1, buf2);
}
END_TEST
...
START_TEST(test_pointer) {
  char buf1[BUF_SIZE], buf2[BUF_SIZE];
  int x = 0;
  sprintf(buf1, "%-15p", (void*)&x);
  s21_sprintf(buf2, "%-15p", (void*)&x);
  ck_assert_str_eq(buf1, buf2);
}
END_TEST
...
START_TEST(test_wide_char_and_string) {
  ...
  sprintf(buf1, "%5lc %-10.3ls", wc, ws);
  s21_sprintf(buf2, "%5lc %-10.3ls", wc, ws);
  ck_assert_str_eq(buf1, buf2);
}
END_TEST
